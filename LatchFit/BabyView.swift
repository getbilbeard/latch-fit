...
struct BabyView: View {
        ...
        NavigationStack {
            GeometryReader { geo in
                ZStack(alignment: .bottom) {
                    ScrollView {
                        ...
                        .padding(.vertical, 12)
                        // keep content clear of the piles bar (matches PilesBar height 172 + padding)
                        .safeAreaPadding(.bottom, 188)
                    }
                ...
                // cache height for fall distance math
            ...
            .safeAreaInset(edge: .bottom) {
                PilesBar(todayWet: todayCount(kind: "wet"), todayDirty: todayCount(kind: "dirty"))
                    ...
                    .zIndex(1) // below toolbar, above scroll content
            }
    ...
    private func logDiaper(kind: String) {
        ...
        fallKind = kind
        fallID = UUID()
        let width = max(320, containerHeight > 0 ? UIScreen.main.bounds.width : UIScreen.main.bounds.width)
        // we still need width; prefer safe margin (use geometry width if you prefer to inject it too)
        fallX = CGFloat.random(in: 60...(width - 60))
        fallStart = -120
        // land just above the piles bar: bar is ~172 high + 12 padding
        fallEnd = max(140, containerHeight - 184)
